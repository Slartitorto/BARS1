			...
			if ((data < min_ok) or (data > max_ok)) {
				...
				if ((armed == 1) and (alarmed == 0)){
					...
					for (int i=0;i<x;i++){
						printf("email n. %d = %s\n\r", i, email[i]);
						printf("send mail \n\r");
						sprintf(mail_command,"echo \"Allarme da sensore di temperatura %s %s; temperatura rilevata = %.2f - out of range (min = %d - max %d)\"|mail -r Sensors -s \"Allarme %s %s\" %s",device_name,position,data,min_ok,max_ok,device_name,position,email[i]);
